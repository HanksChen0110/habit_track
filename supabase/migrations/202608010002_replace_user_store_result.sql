...
create function public.replace_user_store(candidate jsonb)
returns jsonb
language plpgsql
security invoker
set search_path = ''
  ...
  select pg_catalog.jsonb_build_object(
      ...
      (
        select pg_catalog.jsonb_agg(
          pg_catalog.jsonb_build_object(
            ...
            'targetPerDay', stored_habit.target_per_day,
            'createdOn', stored_habit.created_on,
            'archivedOn', stored_habit.archived_on
          ) order by stored_habit.id
        )
        from public.habits as stored_habit
        where stored_habit.user_id = account_id
      ),
      '[]'::jsonb
    ),
    'completions', coalesce(
      (
        select pg_catalog.jsonb_agg(
          pg_catalog.jsonb_build_object(
            'habitId', stored_completion.habit_id,
            'date', stored_completion.date,
            'count', stored_completion.count
          ) order by stored_completion.habit_id, stored_completion.date
        )
        from public.completions as stored_completion
        where stored_completion.user_id = account_id
      ),
      '[]'::jsonb
    )
...
-- Manual rollback requires restoring the prior void-returning function definition.
